...
public class GrafoListaAdyacencia {
    ...
    protected ListaEncadenada vertices; //lista de vertices
    ...
    private boolean marcarYEnpilarVerticesAdyacentes(ListaEncadenada marcados, ListaPila pila,
        ...
        while(listaVerticeActualmenteProceado.hayElementos()==true){
            ...
            if(marcados.buscar(verticeAdyacente.getDescripcion())==null){ //no está en el lisado de marcados
                ...
                pila.agregar(verticeAdyacente); //lo metemos en la pila
            }
        }
        return false;
    }
    ...
    //----------
    /**
     * Este método muestra un grafo conexo
     * @param origen Vertice inicial.
     * @return Regresa true en caso de que sea conexo, false en caso contrario.
     */
    public boolean GConexo(Object origen){
        ListaEncadenada verticesYaAnalizados= new ListaEncadenada();
        ListaPila pila=new ListaPila();
        ListaEncadenada marcados=new ListaEncadenada();
        pila.agregar(origen);
        Integer numeroVertice = (Integer)vertices.regresarFrente();
        verticesYaAnalizados.insertar(numeroVertice,1);
        while(pila.vacia()==false){
            Object visitado=pila.quitar();
            for(int contador=0;contador<vertices.contarElementos();contador++){
                if(marcarYEnpilarVerticesAdyacentes(verticesYaAnalizados,pila, marcados)){
                    pila.agregar(vertices.obtener(contador));
    ...
    /**
     * Este método ayuda a decir si en todos los elementos estan marcados
     * @param verticesYaAnalizados
     * @return
     */
    private boolean estanTodosMarcados(ListaEncadenada verticesYaAnalizados){
