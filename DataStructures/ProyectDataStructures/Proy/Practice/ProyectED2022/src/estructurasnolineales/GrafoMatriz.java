...
import registros.grafos.Vertice;
/**
 * Clase GrafoMatriz
 * @author Profesor Aldonso Becerra Sanchez
 * @version 1.0
 */

/**
 * Clase GrafoMatriz
 * @author Alumno : Daniel Alejandro Morales Castillo
 * @version 3.0
 */
...
public class GrafoMatriz {
    ...
    public ListaEncadenada recorridoProfundidad(Object origen){
        ...
        if(indiceVerticeOrigen==null){ //el origen no existem no se puede hacer un reocrrido
            ...
            //mete en una pila.
            //Crear un lote de marcados contodo en falso
            marcados.rellenar(false,vertices.cantidadElementos());
            //Mqrcar este vértice
     ...
     * @param vertice Nombre del vertice a eliminar.
     * @return Regresa true en caso de que se elimine de manera correcta, false en caso contrario.
     */
    public boolean eliminarVertice(Object vertice){
        Integer numVertice = (Integer)vertices.buscar(vertice);
        if(numVertice!=null){
            for(int numVerticeOrigen=0;numVerticeOrigen<aristas.getFilas();numVerticeOrigen++){
                if ( ((double)aristas.obtenerElementoCelda(numVerticeOrigen,numVertice)) != 0.0
                        && aristas.obtenerElementoCelda(numVerticeOrigen,numVertice) != null ){
                    aristas.asignarCelda(numVerticeOrigen, numVertice, 0.0);
                }
            }
            Tabla2D nueva = new Tabla2D(vertices.cantidadElementos()-1,vertices.cantidadElementos()-1);
            int renglones =0;
            int columnas =0;
            for(int fil=0;fil<aristas.getFilas();fil++){
                if(fil!=numVertice){
                    for(int col=0;col<aristas.getColumnas();col++){
                        if(col!=numVertice){
                            nueva.asignarCelda(renglones,columnas,aristas.obtenerElementoCelda(fil,col));
                        }
                        columnas++;
        ...
        else{//No hay nodo a eliminar.
            return false;
        }
    }

    /**
     * Indica si dos vertices son adyacentes.
     * @param origen Vértice origen.
     * @param destino Vértice destino.
     * @return Regresa true en caso de que exista adyacencia, false en caso contrario.
     */
    public boolean esAdyacente(Object origen, Object destino){
        Integer numVerticeOrigen = (Integer)vertices.buscar(origen);
        Integer numVerticeDestino = (Integer)vertices.buscar(destino);
        if (numVerticeOrigen != null && numVerticeDestino != null) {
            if( ((double)aristas.obtenerElementoCelda(numVerticeOrigen,numVerticeDestino)) != 0.0
                    && aristas.obtenerElementoCelda(numVerticeOrigen,numVerticeDestino) != null ){
                return true;
            }
            else{
                return false;
     ...
     * @param origen ver origen.
     * @param destino ver destino.
     * @return Regresa verdadero si se elimino de manera correcta, false en caso contrario.
     */
    public boolean eliminarArista(Object origen, Object destino){
        ...
        if (numVerticeOrigen != null && numVerticeDestino != null) {
            if( ((double)aristas.obtenerElementoCelda(numVerticeOrigen,numVerticeDestino)) != 0.0
                    && aristas.obtenerElementoCelda(numVerticeOrigen,numVerticeDestino) != null ){
                aristas.asignarCelda(numVerticeOrigen,numVerticeDestino,0.0);
                return true;
            }
            else{
                return false;
    ...
    /**
     * Indica si es pseudografo.
     * @return Regresa true en caso de que lo sea, false en caso contrario.
     */
    public boolean esPseudografo(){
        boolean condicional = false;
        for(int fil=0;fil<aristas.getFilas();fil++){
            if(aristas.obtenerElementoCelda(fil,fil)!=null && (double)aristas.obtenerElementoCelda(fil, fil)!=0.0){
                condicional=true;
            }
        }
        return condicional;
    ...
    /**
     * Indica si es multigrafo.
     * @return Regresa true en caso de que lo sea, false en caso contrario.
     */
    public boolean esMultigrafo(){
        boolean condicional = false;
        for(int reng=0;reng<aristas.getFilas();reng++){
            for(int col=0;col<aristas.getColumnas();col++){
                double valor1 = (double)aristas.obtenerElementoCelda(reng,col);
                double valor2 = (double)aristas.obtenerElementoCelda(col,reng);
                if(valor1==valor2 && valor1!=0.0 & valor2!=0.0){
                    condicional=true;
                }
            }
        }
    ...
    /**
     * Imprime el numero aristas que tiene un vértice.
     * @param nombreVertice Vértice a revisar.
     * @return Regresa el número de aristas, en caso de que no exista regresa un -1.
     */
    public int gradoVertice(Object nombreVertice){
        int grado =0;
        Integer numVertice = (Integer)vertices.buscar(nombreVertice);
        if (numVertice!=null){
            for(int numVerticeDestino=0;numVerticeDestino<aristas.getColumnas();numVerticeDestino++){
                if ( ((double)aristas.obtenerElementoCelda(numVertice,numVerticeDestino)) != 0.0
                        && aristas.obtenerElementoCelda(numVertice,numVerticeDestino) != null ){
                    grado++;
                }
            }
            return grado;
    ...
    /**
     * Este método muestra si un grafo es conexo o no lo es.
     * @param verticeInicial Vertice inicial.
     * @return Regresa true en caso de que sea conexo, false en caso contrario.
     */
    public boolean esConexo(Object verticeInicial){
        ArregloDatos verticesYaAnalizados= new ArregloDatos(vertices.cantidadElementos());
        verticesYaAnalizados.rellenar(0, vertices.cantidadElementos());
        ArregloCola cola = new ArregloCola(vertices.cantidadElementos());
        cola.poner(verticeInicial);
        Integer numeroVertice = (Integer)vertices.buscar(verticeInicial);
        verticesYaAnalizados.insertar(numeroVertice,1);
        while(cola.vacio()==false){
            Object visitado=cola.quitar();
            for(int contador=0;contador<vertices.cantidadElementos();contador++){
                if(esAdyacente(visitado,vertices.obtener(contador)) && (Integer)verticesYaAnalizados.obtener(contador)==0){
                    cola.poner(vertices.obtener(contador));
                    verticesYaAnalizados.insertar(contador,1);
                }
            }
        }
        boolean conexo = estanTodosMarcados(verticesYaAnalizados);
        return conexo;
    }

    /**
     * Este método ayuda a decir si en un arreglo todos los elementos estan marcados
     * @param verticesYaAnalizados
     * @return
     */
    private boolean estanTodosMarcados(ArregloDatos verticesYaAnalizados){
        boolean todosMarcados = true;
        for (int contador =0;contador<vertices.capacidad();contador++){
            if((Integer)verticesYaAnalizados.obtener(contador)==0){
                todosMarcados=false;
            }
        }
    ...
    /**
     * Busca un elemento haciendo uso de un recorrido
     * @param buscar Vertice a buscar.
     * @return Regresa el elemento o null en caso de que no lo encuentre.
     */
    public Object buscarVertice(Object buscar){
        ArregloDatos verticesYaAnalizados= new ArregloDatos(vertices.capacidad());
        verticesYaAnalizados.rellenar(0, vertices.cantidadElementos());
        ArregloPila pila = new ArregloPila(vertices.cantidadElementos());
        Object verticePartida=vertices.obtener(0);
        pila.poner(verticePartida);
        Integer numeroVertice = (Integer)vertices.buscar(verticePartida);
        verticesYaAnalizados.insertar(numeroVertice,1);
        Object encontrado = null;
        while(pila.vacio()==false){
            Object visitado=pila.quitar();
            if(visitado.toString().equalsIgnoreCase(buscar.toString())){
                encontrado=visitado;
            }
            for(int contador=0;contador<vertices.cantidadElementos();contador++){
                if(esAdyacente(visitado,vertices.obtener(contador)) && (Integer)verticesYaAnalizados.obtener(contador)==0){
                    pila.poner(vertices.obtener(contador));
                    verticesYaAnalizados.insertar(contador,1);
                }
            }
        }
    ...
    public ArregloDatos listarVertices(){
        return vertices;
    }

    public Object listarAristas(Object vertice){return aristas;}

    /**
     *Recorrido de un grafo para ver si hay camino cerrado
     * @param origen Vertice inicial o de partida.
     */
    public void hayCaminoCerrado(Object origen){
        ArregloDatos verticesYaAnalizados= new ArregloDatos(vertices.cantidadElementos());
        verticesYaAnalizados.rellenar(0, vertices.cantidadElementos());
        ArregloCola cola = new ArregloCola(vertices.cantidadElementos());
        cola.poner(origen);
        Integer numeroVertice = (Integer)vertices.buscar(origen);
        verticesYaAnalizados.insertar(numeroVertice,1);
        while(cola.vacio()==false){
            Object visitado=cola.quitar();
            SalidaTerminal.consola(visitado.toString()+" ");
            for(int contador=0;contador<vertices.capacidad();contador++){
                if(esAdyacente(visitado,vertices.obtener(contador)) && (Integer)verticesYaAnalizados.obtener(contador)==0){
                    cola.poner(vertices.obtener(contador));
                    verticesYaAnalizados.insertar(contador,1);
    ...
    //ALGORITMO DE PRIM
    /**
     * Condigue la distancia minima entre las aristas.
     */
    public void distanciaMinimaPrim(){
        for(int K = 1;K<vertices.cantidadElementos();K++){
            for(int I = 1;I<vertices.cantidadElementos();I++){
                for(int J = 1;J<vertices.cantidadElementos();J++){
                    if((Double)aristas.obtenerCelda(I, K)+(Double)aristas.obtenerCelda(K, J)<(Double)aristas.obtenerCelda(I, J)){
                        aristas.asignarCelda(I,J,(Double)aristas.obtenerCelda(I, K)+(Double)aristas.obtenerCelda(K, J));
                    }
                }
            }
        }
    ...
    /**
     * Condigue la distancia minima entre los vertices y los almacena
     * @return Regresa una matriz con los datos almacenados.
     */
    public Tabla2D distanciaMinimaYAlmacenajePrim(){
        Tabla2D matrizP = new Tabla2D(vertices.cantidadElementos(),vertices.cantidadElementos());
        for(int K = 1;K<vertices.cantidadElementos();K++){
            for(int I = 1;I<vertices.cantidadElementos();I++){
                for(int J = 1;J<vertices.cantidadElementos();J++){
                    if((Double)aristas.obtenerCelda(I, K)+(Double)aristas.obtenerCelda(K, J)<(Double)aristas.obtenerCelda(I, J)){
                        aristas.asignarCelda(I,J,(Double)aristas.obtenerCelda(I, K)+(Double)aristas.obtenerCelda(K, J));
                        matrizP.asignarCelda(I,J,K);
                    }
                }
            }
        }
    ...
    /**
     * Metodo que calcula el algoritmo de prim.
     * @return Regresa un grafo con el recorrido de prim.
     */
    public void algoritmoPrim(){
        Tabla2D matrizP = new Tabla2D(vertices.cantidadElementos(),vertices.cantidadElementos());
        ArregloDatos visitados = new ArregloDatos(vertices.cantidadElementos());
        int u =0;
        int v =0;
        double costominimo = 3.0;
        double total = 0.0;
        for(int i = 0; i<visitados.cantidadElementos(); i++ ){
            visitados.inicializar(0);
            for (int j =0; j<visitados.cantidadElementos(); j++){
                if((Double)matrizP.obtenerCelda(i, j) == 0){
                    matrizP.asignarCelda(i,j,3.0);
                }
            }
        }
        visitados.insertar(0,1);

        for(int contador = 0; contador<visitados.cantidadElementos()-1; contador++){
            costominimo = 3.0;
            for(int i = 0; i<vertices.cantidadElementos(); i++ ){
                for(int j=0; j<vertices.cantidadElementos(); j++){
                    if(visitados.insertar(j,0)){
                        if(costominimo>(Double)matrizP.obtenerCelda(i,j)){
                            matrizP.asignarCelda(i,j,costominimo);
                            u = i;
                            v = j;
                        }
                    }
        ...
        visitados.insertar(v,1);
        total = total+costominimo;
        SalidaTerminal.consola("origen" + u + "--->" + v + "destino :" + costominimo);
        SalidaTerminal.consola("El total del peso del grafo seria: " + total);
    }
